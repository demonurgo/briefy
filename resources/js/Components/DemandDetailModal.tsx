...
import { ChevronDown, Download, Edit2, Link2, Paperclip, Pencil, Plus, Save, Send, Trash2, X } from 'lucide-react';
...
export function DemandDetailModal({ demand, isAdmin, teamMembers, onClose }: Props) {
  ...
  const { auth } = usePage<{ auth: { user: User }; [key: string]: unknown }>().props;
      ...
      >
        ...
        <div className="grid flex-1 grid-cols-1 gap-0 overflow-hidden md:grid-cols-3">
          ...
          <div className="flex flex-col overflow-hidden md:col-span-2">
            <div className="border-b border-[#e5e7eb] bg-white px-6 py-3 dark:border-[#1f2937] dark:bg-[#111827]">
              <span className="text-sm font-semibold text-[#111827] dark:text-[#f9fafb]">{t('demands.comments')}</span>
            </div>
