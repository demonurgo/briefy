...
// Rendered via Portal at body level — escapes all stacking contexts
function NavTooltip({ tooltip }: { tooltip: TooltipState | null }) {
  if (!tooltip) return null;
  return createPortal(
    <div
      className="pointer-events-none fixed z-[9999] whitespace-nowrap rounded-[6px] bg-[#111827] px-2.5 py-1 text-xs font-medium text-white shadow-lg dark:bg-[#f9fafb] dark:text-[#111827]"
      style={{ left: tooltip.x, top: tooltip.y, transform: 'translateY(-50%)' }}
    >
      <span
        className="absolute right-full top-1/2 -translate-y-1/2 border-4 border-transparent border-r-[#111827] dark:border-r-[#f9fafb]"
      />
      {tooltip.label}
    </div>,
    document.body,
        ...
        <div className={`flex h-full flex-col ${collapsed ? 'items-center px-2 py-4' : 'p-4'}`}>
          ...
          <nav className={`flex-1 ${collapsed ? 'flex flex-col items-center gap-1 w-full' : 'space-y-1'}`}>
            {navItems.map(({ key, icon: Icon, href }) => (
              <Link
                ...
                onMouseEnter={e => showTooltip(e, t(key))}
          ...
          <div className={`flex flex-col ${collapsed ? 'items-center gap-1 w-full' : 'space-y-1'}`}>
            <Link
              ...
              onClick={e => e.stopPropagation()}
              onMouseEnter={e => showTooltip(e, t('nav.settings'))}
              onMouseLeave={hideTooltip}
            >
