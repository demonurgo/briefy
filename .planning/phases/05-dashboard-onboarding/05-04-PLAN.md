...
must_haves:
  truths:
    - "Admin vê toggle 'Visão pessoal / Visão geral' no header; colaborador NÃO vê o toggle"
    - "Visão pessoal exibe: 5 status cards, 'Seu foco agora', bar chart semanal, donut 'Seu progresso', tabela 'Minhas demandas' com tabs, card 'Bloqueios / Aguardando', ActivityFeed"
    - "Visão geral exibe: 5 status cards org-wide, donut 'Panorama geral', bar chart 'Demandas por prioridade', line chart 'Demanda ao longo do tempo', tabela 'Últimas demandas' com UserAvatar, donut 'Desempenho da equipe', ActivityFeed"
    - "Date range picker (visão geral) dispara router.get com only: ['overview'] ao mudar; default = semana atual"
    - "OnboardingChecklist aparece no topo quando hasClients=false ou hasDemands=false"
    - "DashboardPlanningWidget existente permanece renderizado (não removido)"
    - "Todos os gráficos Recharts usam as cores do STATUS_COLORS e PRIORITY_COLORS do UI-SPEC"
  artifacts:
    - path: "resources/js/pages/Dashboard.tsx"
      provides: "Página completa do dashboard com views personal + overview"
      min_lines: 350
  key_links:
    - from: "resources/js/pages/Dashboard.tsx"
      to: "app/Http/Controllers/DashboardController.php"
      via: "Inertia props: personal, overview, activityFeed, hasClients, hasDemands"
      pattern: "personal.*overview.*activityFeed"
    - from: "resources/js/pages/Dashboard.tsx"
      to: "resources/js/Components/DashboardStatusCard.tsx"
      via: "import + renderização nos dois views"
      pattern: "DashboardStatusCard"
    - from: "resources/js/pages/Dashboard.tsx"
      to: "resources/js/Components/OnboardingChecklist.tsx"
      via: "hasClients + hasDemands props"
      pattern: "OnboardingChecklist"
---

<objective>
Wave 2 (paralela com 05-03) — Página Dashboard.tsx completa.

Purpose: Substitui o stub atual (que mostra "Em breve...") pelo dashboard completo. Compõe todos os componentes criados em 05-03 e os charts Recharts inline. Este é o maior arquivo da fase — ~400 linhas justificados pela riqueza de dados do dashboard.

Output:
- Dashboard.tsx completo com view pessoal + view gerencial
- Todos os gráficos Recharts (PieChart, BarChart, LineChart)
- Toggle de view, date range picker, tabelas com tabs
- Integração com todos os componentes do 05-03
</objective>
...
<interfaces>
<!-- Dashboard.tsx atual (stub a ser substituído) -->
```tsx
// resources/js/pages/Dashboard.tsx — estado atual
interface Props {
...
export default function Dashboard({ planningReminderClients }: Props) {
  // mostra apenas DashboardPlanningWidget + "Em breve..."
}
```

<!-- Props do DashboardController (05-01) -->
```tsx
interface Props {
  planningReminderClients?: Array<{ id: number; name: string; planning_day: number }>;
  personal: {
    statusCounts: Record<string, number>;      // {todo: 3, in_progress: 5, ...}
    deltaVsYesterday: Record<string, number>;  // {todo: 1, in_progress: -2, ...}
    focusDemands: FocusDemand[];               // max 5
    myDemands: Demand[];                       // max 20
    blockers: Blocker[];                       // awaiting_feedback
    weekProgress: { completed: number; total: number };
    completedByDay: Record<string, number>;    // {'Seg': 2, 'Ter': 0, ...}
  };
  overview: {
    statusBreakdown: Record<string, number>;
    deltaVsYesterday: Record<string, number>;
    overdueCount: number;
    priorityBreakdown: Record<string, number>; // {high: 5, medium: 10, low: 3}
    demandsOverTime: Array<{ date: string; created: number; completed: number }>;
    latestDemands: LatestDemand[];             // max 5
...
<!-- STATUS_COLORS e PRIORITY_COLORS do UI-SPEC -->
```tsx
...
<!-- Recharts imports necessários (package instalado na Wave 0) -->
```tsx
import {
...
<!-- Inertia partial reload para date range (Pattern 2 do RESEARCH.md) -->
```tsx
...
<!-- isAdminOrOwner no frontend (padrão da Phase 4) -->
```tsx
...
<task type="auto">
  <name>Task 1: Dashboard.tsx — view pessoal + infraestrutura</name>
  <read_first>
    - resources/js/pages/Dashboard.tsx (stub atual — ver estrutura para não perder planningReminderClients)
    - resources/js/Components/DashboardStatusCard.tsx (props interface criada em 05-03)
    - resources/js/Components/DashboardSectionCard.tsx (props interface criada em 05-03)
    - resources/js/Components/ActivityFeed.tsx (ActivityEvent interface criada em 05-03)
    - resources/js/Components/OnboardingChecklist.tsx (Props criada em 05-03)
    - resources/js/types/index.d.ts (ou similar — verificar PageProps e auth.user.role)
  </read_first>
  <action>
Criar o arquivo `resources/js/pages/Dashboard.tsx` completo. Este é o arquivo central da fase.

**Estrutura geral:**
1. Importações (Recharts, componentes, hooks)
2. Interfaces de tipos (FocusDemand, LatestDemand, etc.)
3. Constantes de cor (STATUS_COLORS, PRIORITY_COLORS, STATUS_LABELS)
4. Funções auxiliares (chartTooltipStyle, getStatusCardConfig)
5. Componentes inline pequenos (DonutChart, ViewToggle, DateRangePicker)
6. Componente principal Dashboard com:
   - PersonalView (view pessoal)
   - OverviewView (view gerencial)
...
```
  ...
  <acceptance_criteria>
    - `grep "import.*recharts" resources/js/pages/Dashboard.tsx` retorna match (Recharts importado)
    - `grep "DashboardStatusCard" resources/js/pages/Dashboard.tsx` retorna match (componente usado)
    - `grep "OnboardingChecklist" resources/js/pages/Dashboard.tsx` retorna match (componente usado)
    - `grep "DashboardPlanningWidget" resources/js/pages/Dashboard.tsx` retorna match (widget preservado)
    - `grep "isAdminOrOwner\|isAdmin.*admin.*owner" resources/js/pages/Dashboard.tsx` retorna match (gate no frontend)
    - `grep "only.*overview" resources/js/pages/Dashboard.tsx` retorna match (partial reload)
    - `grep "ActivityFeed" resources/js/pages/Dashboard.tsx` retorna match
    - `grep "teamWorkload" resources/js/pages/Dashboard.tsx` retorna match (DASH-02 coberto)
    - `grep "clientDistribution" resources/js/pages/Dashboard.tsx` retorna match (DASH-03 coberto)
    - `npx tsc --noEmit 2>&1 | grep "pages/Dashboard"` — sem erros TypeScript
    - Build Vite bem-sucedido: `npm run build 2>&1 | grep -c "error"` retorna 0
  </acceptance_criteria>
  <done>Dashboard.tsx completo com todas as regions, componentes integrados, charts Recharts, toggle de view, date range picker, onboarding checklist; TypeScript e build limpos.</done>
</task>
...
|----------|-------------|
| Frontend toggle admin/overview | Dupla verificação: server-side (overview null) + client-side (isAdmin check). Colaborador jamais vê dados gerenciais mesmo manipulando JS. |
| Date range inputs | Valores de date input sanitizados no backend via isValidDate(); Eloquent usa parameterized bindings. |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-5-01 | Elevation of Privilege | Dashboard.tsx — toggle visível para admin | mitigate | `isAdmin` computed de `auth.user.role` (shared prop server-side); view gerencial só renderiza se `overview !== null` — ambos os checks necessários; colaborador não recebe overview props do servidor |
| T-5-04 | Tampering | DateRangePicker — input type=date | mitigate | Backend valida com `isValidDate()` antes de `Carbon::parse()`; inputs têm type="date" que previne texto arbitrário no browser |
| T-5-09 | Information Disclosure | Dashboard.tsx — dados de outros usuários em personal props | mitigate | personal props scoped a `assigned_to = $user->id` no backend; colaborador não vê dados de demandas de outros usuários |
</threat_model>

<verification>
**Plano 04 completo quando:**
1. `npx tsc --noEmit` — 0 erros TypeScript
2. `npm run build` — build Vite sem erros
3. `grep "recharts" resources/js/pages/Dashboard.tsx` — match
4. `grep "teamWorkload\|clientDistribution" resources/js/pages/Dashboard.tsx` — match (DASH-02 e DASH-03)
5. `php artisan test` — suite completa verde
</verification>

<success_criteria>
- Dashboard.tsx substitui o stub completamente
- View pessoal exibe todos os elementos do D-07 ao D-11 com dados reais
- View gerencial exibe todos os elementos do D-14 ao D-19 com charts Recharts
- DASH-02 (workload por membro) e DASH-03 (distribuição por cliente) visíveis na view gerencial
- DashboardPlanningWidget preservado
- OnboardingChecklist integrado
- TypeScript e build limpos
</success_criteria>

<output>
Após conclusão, criar `.planning/phases/05-dashboard-onboarding/05-04-SUMMARY.md`
</output>
